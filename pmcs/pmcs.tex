...
choosing a \emph{pivot} element $a$. Then proceeds by comparing every other element to $a$,
...
\caption{Quicksort}
    \begin{algorithmic}[1]
        ...
        \Else
            \State Choose $a \in S$ uniformly at random (the \emph{pivot})
...
\begin{proof}[First proof of Theorem~\ref{the:quicksort} by recursion]
    ...
    Next we use induction to show that $C(n) \leq 5n \log n$.
\end{proof}

\begin{proof}[Second proof of Theorem~\ref{the:quicksort} by indicator random variables]
